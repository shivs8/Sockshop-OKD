...
export CARTS_PODS_AVAILABLE=$(oc get deployments/carts -n production -o json | jq '.status.readyReplicas')
...
do
    ...
    sleep 30
    CARTS_PODS_AVAILABLE=$(oc get deployments/carts -n production -o json | jq '.status.readyReplicas')
...
export CARTS_URL=$(oc describe svc carts -n production | grep "LoadBalancer Ingress:" | sed 's/LoadBalancer Ingress:[ \t]*//')
